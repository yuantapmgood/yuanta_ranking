...
# --- 初始化 Session State ---
# 用來暫存使用者上傳並解析完成的原始資料，避免每次操作都要重傳
if 'raw_data' not in st.session_state:
    st.session_state.raw_data = None

# --- 資料清理與名稱整合函數 ---
@st.cache_data # 快取這個函式，加速資料清理過程
def process_raw_data(uploaded_file):
    try:
        ...
        df['投信'] = df['基金名稱'].str[:2] # 簡易萃取投信名稱
        return df
    except Exception as e:
        st.error(f"檔案解析失敗，請確認是否為投信公會下載的格式。錯誤訊息: {e}")
        return None
...
# ==========================================
# 側邊欄 (Sidebar) - 基金設定管理區塊
# ==========================================
with st.sidebar:
    st.header("⚙️ 基金複委託設定管理")
    st.info("此區的修改會即時套用到右側的計算結果中。")
...
# ==========================================
# 1. 檔案上傳區
uploaded_file = st.file_uploader("請上傳當月投信公會 Excel 檔", type=['xls', 'xlsx'])

# 當有新檔案上傳時，解析並存入 session_state
...
# 2. 資料處理與呈現 (只要 session_state 裡有資料就執行)
if st.session_state.raw_data is not None:
    ...
    if not new_funds.empty:
        ...
        if not sub_broker_df.empty:
            ...
            with col1:
                # 動態抓取資料中存在的券商，並將「元大證券」預設為第一個選項 (如果存在)
